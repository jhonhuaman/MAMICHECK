...
export const loader = async () => {
  ...
  const registros: Registro[] = [
    ...
    { id: 2, pacienteId: 1, Age: 30, SystolicBP: 135, DiastolicBP: 88, BS: 6.5, BodyTemp: 98.6, HeartRate: 74 },
    { id: 3, pacienteId: 1, Age: 35, SystolicBP: 120, DiastolicBP: 80, BS: 5.9, BodyTemp: 98.2, HeartRate: 70 },
    { id: 4, pacienteId: 1, Age: 30, SystolicBP: 150, DiastolicBP: 100, BS: 7.5, BodyTemp: 98.7, HeartRate: 80 },
  ];
  ...
  return (
    <SidebarLayout>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="col-span-2 flex flex-col gap-6 h-[680px]">
          <div className="bg-white p-6 rounded-lg shadow-lg h-[340px] flex flex-col">
            <h2 className="text-2xl font-semibold text-blue-700 mb-4">Nivel de Riesgo</h2>
            {pacienteSeleccionado && <Histogram data={{ ...pacienteSeleccionado, registros: registrosFiltrados }} />}
          </div>

          <div className="bg-white p-6 rounded-lg shadow-lg overflow-auto flex-1">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-semibold text-blue-700">
                Registros {selectedId ? `del paciente #${selectedId}` : ''}
              </h2>
              <button
                className="text-sm text-blue-500 hover:underline"
                onClick={() => navigate(`/record?pacienteId=${selectedId}`)}// Redirigir a la ruta dinámica
              >
                Ver más registros
              </button>
            </div>
            <table className="w-full text-sm text-left border-separate border-spacing-0.5">
              <thead className="bg-gray-100 text-gray-700 border-b">
                <tr>
                  ...
                  <th className="px-3 py-3 text-base">Edad</th>
                  <th className="px-3 py-3 text-base">Sistólica</th>
                  <th className="px-3 py-3 text-base">Diastólica</th>
                  <th className="px-3 py-3 text-base">Glucosa (BS)</th>
                ...
                {registrosFiltrados.length > 0 ? (
                  registrosFiltrados.map((reg, idx) => (
                    <tr key={reg.id} className="border-t hover:bg-blue-50">
                      ...
                      <td className="px-3 py-2">{reg.Age}</td>
